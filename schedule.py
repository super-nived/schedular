...
def choose_scheduling_approach(jobs, machines, downtimes, scheduled_jobs=None, approach='mixed'):
    ...
    if approach.lower() == 'mixed':
        ...
        if len(result) == 6:  # Full mixed result
            solver, tasks, forward_jobs, backward_jobs, unscheduled_jobs, schedule_result = result
            results['mixed'] = {
                'solver': solver,
                'tasks': tasks,
            ...
            results['mixed'] = {'solver': None, 'tasks': None, 'schedule_result': {}}
        ...
        if len(result_mixed) == 6:
            solver_m, tasks_m, forward_jobs, backward_jobs, unscheduled_jobs, schedule_result_m = result_mixed
            results['mixed'] = {
                'solver': solver_m,
                'tasks': tasks_m,
                'forward_jobs': forward_jobs,
                'backward_jobs': backward_jobs,
                'unscheduled_jobs': unscheduled_jobs,
                'schedule_result': schedule_result_m
            }
        else:
            results['mixed'] = {'solver': None, 'tasks': None, 'schedule_result': {}}
        ...
        solvers = [('Mixed', results.get('mixed', {}).get('solver')), ('Forward', solver_f), ('Backward', solver_b)]
        tasks_dict = [('Mixed', results.get('mixed', {}).get('tasks')), ('Forward', tasks_f), ('Backward', tasks_b)]

        for name, solver in solvers:
            if solver:
                tasks = next((tasks for task_name, tasks in tasks_dict if task_name == name), None)
                if tasks:
                    makespan = max([solver.Value(end_var) for (_, end_var, _, sel_var) in tasks.values() if solver.Value(sel_var) == 1], default=0)
